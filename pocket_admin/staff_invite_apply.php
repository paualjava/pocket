...
class staff_invite_apply extends base
{
	private $page_size = 20; //一页显示的行数
	private $table_name="pocket_staff_invite";
	function __construct()
	{
		$GLOBALS['smarty']->assign('nav_left',"staff_invite_apply");
	...
	function main()
	{
		...
		$GLOBALS['smarty']->display('staff_invite_apply.htm');
	}
	/**
	 * 添加邀请码
	 *
	...
	function add()
	{
		...
		$tag = $GLOBALS['db']->getAll($sql);
		$GLOBALS['smarty']->assign('tag', $tag);
		$GLOBALS['smarty']->display('staff_invite_apply_add.htm');
	}
	function ajax_add()
	{
...
$staff_invite_apply = new staff_invite_apply();
$sign=@is_callable(array($staff_invite_apply,$act));
if($sign)
$staff_invite_apply->$act();
else
