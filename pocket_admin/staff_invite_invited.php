...
class staff_invite_invited extends base
{
	private $page_size = 20; //一页显示的行数
	private $table_name="pocket_staff_invite";
	function __construct()
	{
		$GLOBALS['smarty']->assign('nav_left',"staff_invite_invited");
	...
	function main()
	{
		...
		$GLOBALS['smarty']->display('staff_invite_invited.htm');
	}
	/**
	 * 添加邀请码
	 *
	...
	function add()
	{
		...
		$tag = $GLOBALS['db']->getAll($sql);
		$GLOBALS['smarty']->assign('tag', $tag);
		$GLOBALS['smarty']->display('staff_invite_invited_add.htm');
	}
	function ajax_add()
	{
...
$staff_invite_invited = new staff_invite_invited();
$sign=@is_callable(array($staff_invite_invited,$act));
if($sign)
$staff_invite_invited->$act();
else
